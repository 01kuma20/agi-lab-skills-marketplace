...
インストール後、Claude Code のチャットで以下のように呼び出す：

```
/career-compass https://www.wantedly.com/projects/xxxxx
```

初回はオンボーディングウィザード（10問）が起動してプロフィールを保存します。
2 回目以降はプロフィールを自動ロードして完全自律実行されます。
...
求人 URL を渡すだけで転職活動を自律支援する plugin です。

| ステップ | 処理内容 |
|---------|---------|
| 1 | プロフィール確認（初回：10問ウィザードでプロフィール生成） |
| 2 | 求人 URL から必須スキル・歓迎スキルを自動取得 |
| 3 | スキルギャップ分析（マッチスコア 0–100 + プログレスバー） |
| 4 | 学習ロードマップ生成（書籍・講座・ハンズオン提案） |
| 5 | 職務経歴書 Markdown を `output/resume_YYYYMMDD.md` に自動保存 |
...
│   │   ├── display_results.sh
│   │   └── progress_bar.sh
│   ├── data/               ← profile.json（初回起動時に生成）
│   ├── output/             ← 生成した職務経歴書の保存先
│   └── skills/
│       └── career-compass/
│           └── SKILL.md
├── hackathon-starter/
